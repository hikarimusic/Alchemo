...
def initialize_docking():
    try:
        ...
        current_state.update({
            ...
            'current_optimizer': 'exploration',  # Start with exploration phase
            'phase_switch_counter': 0,
            'phase_length': 50,  # Switch phases every 50 steps
            'protein_coords_tensor': None,
            'prot_vdw': None,
        ...
        # Set up exploration optimizer (aggressive for broad search)
        exploration_optimizer = torch.optim.SGD([drug_conf.conf_params],
                                lr=0.4,                # More aggressive learning rate
                                momentum=0.98,         # Very high momentum
                                dampening=0,           # No dampening for more aggressive updates
                                weight_decay=0.4,      # More noise through weight decay
                                nesterov=True)         # More aggressive momentum implementation
        
        # Set up refinement optimizer (precise for local optimization)
        refinement_optimizer = torch.optim.Adam([drug_conf.conf_params],
                               lr=0.02,               # Lower learning rate for precision
                               betas=(0.9, 0.999),    # Standard Adam parameters
                               weight_decay=0.01)     # Less noise for fine-tuning
        ...
        # Get protein properties - assume protein_mol is available
        # If not available, simulate properties as a fallback
        if current_state.get('protein_mol'):
            prot_is_hydrophobic, prot_is_hbond_donor, prot_is_hbond_acceptor = scoring_fn.precompute_protein_properties(current_state['protein_mol'])
        else:
            # Fallback logic: Create simulated properties based on atom types
        ...
        current_state.update({
            ...
            'exploration_optimizer': exploration_optimizer,
...
def optimize_docking():
    try:
        ...
        # Select the appropriate optimizer based on current phase
        if current_state['current_optimizer'] == 'exploration':
            optimizer = current_state['exploration_optimizer']
        else:
            optimizer = current_state['refinement_optimizer']
        ...
        # Check round improvements - now based on threshold
        ...
        # Function to switch to refinement phase (start from best conformations)
        def switch_to_refinement():
            current_state['current_optimizer'] = 'refinement'
            current_state['phase_switch_counter'] = 0
            
            # Use global best conformers as starting points for refinement
            if global_best_coords is not None:
                # Get indices of the best conformers
                sorted_indices = torch.argsort(global_best_scores)
                
                # Keep the top 20% of conformers from global best
                top_percentage = 0.2
                n_top = int(drug_conf.n_conformers * top_percentage)
                n_top = max(n_top, 1)  # At least keep 1
                
                # Clone the current parameter tensor
                new_params = drug_conf.conf_params.clone()
                
                # Extract positions, orientations, and rotatable bond angles from best coordinates
                best_coords = global_best_coords[sorted_indices[:n_top]]
                
                # Recreate parameters for top conformers
                # This is a simplified version - ideally, we would perform inverse kinematics
                # to get proper parameters from coordinates, but this is a starting point
                for i in range(n_top):
                    # For simplicity, only copy the translational part (first 3 parameters)
                    # which is the molecular center of mass position
                    center_of_mass = best_coords[i].mean(dim=0)
                    new_params[i, :3] = center_of_mass
                
                # Update parameters with the new values
                drug_conf.conf_params.data = new_params
                
                logger.info(f"Switched to refinement: initialized from top {n_top} conformers")
            else:
                logger.info("Switched to refinement without best conformers initialization")
        
        # Function to switch to exploration phase
        def switch_to_exploration():
            current_state['current_optimizer'] = 'exploration'
            current_state['phase_switch_counter'] = 0
            
            # Optionally reinitialize some conformers to explore new regions
            # Keep top 10% of best conformers, reinitialize the rest
            if global_best_coords is not None:
                # Get indices of the best conformers
                sorted_indices = torch.argsort(global_best_scores)
                
                # Keep the top 10% of conformers from global best
                top_percentage = 0.1
                n_top = int(drug_conf.n_conformers * top_percentage)
                n_top = max(n_top, 1)  # At least keep 1
                ...
                # Reinitialize 90% of conformers randomly
                random_params = drug_conf._initialize_near_surface(0.0)
                
                # Keep top conformers, randomize the rest
                keep_mask = torch.zeros(drug_conf.n_conformers, dtype=torch.bool, device=drug_conf.device)
                keep_mask[sorted_indices[:n_top]] = True
                
                # Update with random parameters except for top conformers
                new_params[~keep_mask] = random_params[~keep_mask]
                drug_conf.conf_params.data = new_params
                
                logger.info(f"Switched to exploration: kept top {n_top} conformers, reinitialized the rest")
            else:
                logger.info("Switched to exploration without conformer reinitialization")
                
        # Dynamic phase switching based on performance
        if current_state['current_optimizer'] == 'exploration' and steps_since_improvement > 5:
            # Switch to refinement phase if exploration is stagnating
            switch_to_refinement()
            logger.info("Switching to refinement optimizer due to exploration stagnation")
        elif current_state['current_optimizer'] == 'refinement' and steps_since_improvement > 10:
            # Switch back to exploration if refinement is stagnating too long
            switch_to_exploration()
            logger.info("Switching to exploration optimizer due to refinement stagnation")
            
        # Regular phase switching after fixed length
        current_state['phase_switch_counter'] += 1
        if current_state['phase_switch_counter'] >= current_state['phase_length']:
            # Switch optimization phase
            if current_state['current_optimizer'] == 'exploration':
                switch_to_refinement()
                logger.info("Switching to refinement optimizer (scheduled)")
            else:
                switch_to_exploration()
                logger.info("Switching to exploration optimizer (scheduled)")
            current_state['phase_switch_counter'] = 0
        
        # Extended stagnation - reinitialize coordinates
        ...
        # Get only the single best conformer if stopped/complete, otherwise show top conformers during optimization
        data = request.json or {}
        is_stopped = data.get('is_stopped', False) or is_complete
        ...
        return jsonify({
            ...
            'current_optimizer': current_state['current_optimizer']  # Include current optimizer in response
        })
